...
1. DATA GENERATION : log/event/생성, 활동, DB ... 데이터(더미, 서비스 통과 생성, 센서)
   데이터 발생
        │
        ▼
2. DATA INGESTION : firehose > kinesis/kafka, E`L`K, E`F`K..서비스 
   데이터 수집 / 유입
        │
        ▼
3. DATA STORAGE : s3 (데이터 레이크)등... -> s3(최종/중간/최초 목적지(데이터 레이크))
   원본 저장
        │
        ▼
4. DATA PROCESSING : ETL, ELT, 메달리온 아키텍처(브론즈, 실버, 골드), pandas/polars/spark -> Transform/Extract, s3(중간 저장)
   정제 / 변환 / 집계
        │
        ▼
5. DATA MODELING : 대시보드용 관제, 인사이트도출, 모델학습,... -> 목적, athena(sql), opensearch(ELK,...)...
   분석 가능한 데이터 구조화
        │
        ▼
...
8. Observability : grafana, prometheus/ ELK에서는 키바나, opensearch 대시보드 등 제품 활용
```

# Data Generation
- 데이터가 어디서 발생하는가?
    - 목표
        - 데이터를 더미로 발생(클라이언트 -> 서비스 -> 생성된 데이터:가정) _> 바로 데이터 레이크(s3) 적제
          - 데이터: 정상, 비정상
        - 이를 위해서 파이썬 구성(로컬, aws ecs + fargate로 구성) 
    - 도메인 결정 -> 실제 데이터 샘플 획득, 불량률 획득 -> 구성(프로젝트)
    - 형태
        - 최종 형태를 더미로 구성하여 적재
          - Python 코드 작성(faker 패키지 활용)
        - 실제 요청을 구성 -> 서비스 통과 -> 결과 -> 적재
          - 최종 프로젝트 형태
    - 방식
        - ECS Fargate + K6 -> 클라이언트의 요청 구현
            - 미니 프로젝트 구성
          ...
          └───────────────┼────────────────┘
                 ...
                 Amazon Data Firehose
                  ...
                  ─────────────────
                       ...
                       Partition
```
- s3 저장 셈플링 - 검색을 위해서 파티션 작업(열단위 검색유리 -> 검색속도 빠름), 저장 형태(파케이 , 압축, json은 안좋음)
...
- Log / Event Ingestion Pipeline
     - 로그, 이벤트를 수집하여 저장
     - 실시간 분석 x, 데이터를 안전하게 쌓는것이 목표 -> 사용시간은 상이함(당일, 한달 후, 주간...)
```
┌─────────────────────────────────────────────────────┐
...
- Batch Processing Ingesction Pipeline
     - 쌓여있는 데이터를 일정 주기로 처리 -> Airflow
     - airflow/MWAA <-> Step Function + Lambda : 서버리스(비용 저렴)
     - 스케줄링(하루에 한번, 매일 12시에, ...)
```
┌─────────────────────────────────────────────────────┐
...
└─────────────────────────────────────────────────────┘
          ...
          ┌───────────┼───────────┐
          ▼           ▼           ▼
       Pandas(소)  Polars(중)   Spark(대) <- 데이터의 처리 규모에 따라 분류
     (10GB 이내)  (수십 GB)    (수백GB, TB)
          │           │           │
          └───────────┼───────────┘
                      ▼
                 S3 SILVER
...
- kafka/MSK, kinesis, (ELK, EFK, opensearch: 참고) -> 전송
...
# 중간 정리 : 구조 유형(텍스트)
- Application log
     - app, web, IoT(센서), Factory(센서), 게임, 금융
- Business event
     - 행위(로그인, 검색, 스크롤)
- Database Change
     - 업데이트, 수정, 삭제...
...
- 수집한 데이터를 어디에 저장하는가?
     - 원본을 잃지 않고 저장
     - 수집시, 장애등 이 누락 -> 백필 작업(누락분을 다시 수집해 채움)
- 기간
     - 일정 기간만 보관(통산 7일, 조정 가능)
          - kafka, kinesis
     - 영구 보관
          - 데이터 레이크 -> s3 (객체 저장, 버킷 단위로 관리, 검색 지원)
- 데이터 저장 형태 변화
     - csv -> json -> parquet
...
│       └── *.csv
│
├── silver/ (클리닝, 전처리, ..1차 가공된 데이터, 열 기준)
│   ├── orders/
│   │   └── *.parquet
│   └── customers/
│       └── *.parquet
...
```
       ...
       Batch Processing                  Stream Processing
             │                                │
             ▼                                ▼
Airflow/MWAA or Step Functions + Lambda     Spark Streaming
             │                               / Flink
             ▼
   Pandas / Polars / Spark -> 데이터 처리

- Airflow 내부에서 DAG를 구성하고, DAG 내에서 Pandas/Polars/Spark(pySpark)등을 활용하여 TASK 작성 -> 처리
     - EC2등 서버 구축 -> 평시 비용 발생, 인프라 외부 -> 보안 이슈 발생
     - MWAA
- Step Function에서 Lambda를 구성하고 Lambda 내부에서 Pandas/Polars/Spark(pySpark)등을 활용하여 TASK 작성 -> 처리
     - 서버 리스 -> 사용할 때만 비용 발생
```

- Processing 전략(strategy) : ELT, ETL
     - 적절하게 데이터 파이프라인내에 사용
     - ETL: 전통적인 사용법
     - ELT: 새롭게 등장해 확산중
```
...
- 메달리온 아키텍처 구조에서 사용 -> 각 레이어에서 데이터 형태 설명
```
┌─────────────────────────────────────────────────────┐
│                 04. ETL PIPELINE                    │
└─────────────────────────────────────────────────────┘
             ...
             Source / S3 BRONZE
                      │
                  Extract <- Pandas / Polars / Spark / Glue(etl, 데이터베이스, 테이블, 크롤러 등 활용)
                      │
                      ▼
                  Transform <- Pandas / Polars / Spark / Glue(etl, 데이터베이스, 테이블, 크롤러 등 활용)
                      │
                      ▼
                   Load <- Pandas / Polars / Spark / Glue(etl, 데이터베이스, 테이블, 크롤러 등 활용)
                      │
                      ▼
                 S3 SILVER 
                      │
...
- ELT Pipeline
- 금융 프로젝트 활용, 저장 후 SQL 변환 처리, 제조 활용 가능
```
┌─────────────────────────────────────────────────────┐
│                 05. ELT PIPELINE                    │
└─────────────────────────────────────────────────────┘
...
# Data Modeling / 아키텍처
- 데이터를 어떻게 계층화 관리 할 것인가?
- 메달리온 아키텍처가 하나의 대안으로 제시 -> 적용 할 것인지는 선택
- Data Processing의 결과를 관리하는 아키텍처
```
...
```
  ...
  ┌───────────────┼────────────────┐
  ▼               ▼                ▼
Athena(서버리스)  Redshift        OpenSearch(엘라스틱 서치(검색엔진의 오프소스 버전))
  │               │                │
  ▼               ▼                ▼
...
# Orchestration
- 전체 파이프라인 컨트롤(통제)
- Pipeline Control plane: Airflow/MWAA or Step Function -> 스케줄링, 반복, 전방위 컨트롤
- 메달리온 아키텍처 관점 적용
```
                  ORCHESTRATION
